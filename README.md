...
```
config.php          数据库连接 + 登录密码 + 营业时段 + 套餐清单（唯一需要修改的文件）
login.php           登录页
...
开台超过 `open_table_warn_hours`（默认 4）小时仍未结账的会把时长标红，
方便发现忘了关的台。
...
规则写在 `config.php`，按**桌号**匹配，大小写不敏感，`*` 是通配符：
...
> `config.php` 是你自己维护的（里面有数据库密码），升级程序时通常不会跟着换。
> 所以**这一项没写时会自动套用上面的默认值** `['Llevar*', '外带*']`，
> 不会因为漏配就静默失效；页面底部会注明当前用的是默认值还是你的配置。
> 真要「所有台都核对」，请明确写成空数组 `[]`。

也可以按 `order_head.eat_type` 排除（比桌号更准，但要先确认本店的取值）：

```php
'no_combo_eat_types' => [3],   // 拿不准就留空 []
```
...
- 默认 `ack_hours`（6）小时后自动失效；**退出登录即清空**；换一台设备要重新确认
...
由 `config.php` 的 `combo_item_ids` 决定，默认是从菜单表查出来的 6 个自助餐套餐：
